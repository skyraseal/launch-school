...
# 9
# Q: Write your own version of the rails titleize implementation
# A:
def titleize(title)
...
# 10
# Q: Modify the hash such that each member of the Munster family 
# has an additional "age_group" key that has one of three values describing 
...
# A:
munsters.each do |person, demographic|
  demographic['age_group'] = if demographic['age'] <= 18
                               'kid'
                             elsif demographic['age'] < 60
                               'adult'
                             else
                               'senior'
                             end
